...
class Sensor(Actor):
    ...
    def add_data(self,data):
        # 부모(attach_to)가 없을 수 있으므로 안전하게 처리
        try:
            parent = getattr(self.actor, 'parent', None)
            if parent is not None:
                parent_tf = parent.get_transform()
            else:
                parent_tf = self.actor.get_transform()
        except Exception:
            parent_tf = self.actor.get_transform()
        self.data_list.append((parent_tf, data))
