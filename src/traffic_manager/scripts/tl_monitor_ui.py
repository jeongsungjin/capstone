...
def get_light_states(phase_name: str) -> Dict[str, bool]:
    ...
    if phase_name == "P1_MAIN_GREEN":
        state.update({"M_LR_R": False, "M_LR_G": True, "M_RL_R": False, "M_RL_G": True})
    elif phase_name == "P1_YELLOW":
        state.update({"M_LR_R": False, "M_LR_Y": True, "M_RL_R": False, "M_RL_Y": True})
    elif phase_name == "P2_MAIN_LEFT":
        state.update({"M_LR_R": True, "M_LR_LEFT": True, "M_RL_R": True})
    elif phase_name == "P2_YELLOW":
        state.update({"M_LR_R": True, "M_LR_Y": True, "M_RL_R": True})
...
class TrafficLightMonitorUI:
    def __init__(self) -> None:
        ...
        self._intersection_layout = {
            "A": {"pos": (450, 160), "angle": 180, "label": "Intersection 1"},
            "B": {"pos": (70, 600), "angle": -90, "label": "Intersection 2"},
            "C": {"pos": (880, 600), "angle": 90, "label": "Intersection 3"},
        }
    ...
    def _draw_map_view(self, intersections: Dict[str, Dict]) -> None:
        ...
        for iid, cfg in self._intersection_layout.items():
            ...
            self.screen.blit(text, (cfg["pos"][0], max(8, cfg["pos"][1] - 40)))
    ...
    def run(self) -> None:
        while not rospy.is_shutdown() and self._running:
            ...
            if self.show_map_view:
                self._draw_map_view(inter_copy)
                panel_origin_y = self.map_height + self.panel_margin
            items = list(inter_copy.items())
